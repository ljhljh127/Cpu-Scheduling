...
class FCFS {
    ...
    public FCFS(Data[] p, int ProcessCount) {
    ...
    Queue<Data> ReadyQueue = new LinkedList<>();
    ...
    {
        queuefirst.qfcb=(int) (Math.random() * queuefirst.CpuBurstTime) + 1;//랜덤화
        if(queuefirst.IOBurstTime==0)
        {
            queuefirst.qfib=0;
        }
        else
        {
            queuefirst.qfib = (int) (Math.random() * queuefirst.IOBurstTime)+1 ;//랜덤화
        }

    }
    public void fcfsRunning()
    {
        int x=0;
        while(true)
        {
            ...
            {
                if(queuefirst.NeedCpuTime<=0)//needCPu타임을 충족 했을때
                {
                    ...
                    {
                        CpuQueue.add(ReadyQueue.remove());
...
public class FCFSScheduling
{
    ...
    public static void main(String args[])
    {
        ...
        FCFS fcfs=new FCFS(data,ProcessCount);
        fcfs.Arrivalsort();
        fcfs.fcfsRunning();
